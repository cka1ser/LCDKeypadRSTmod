...
LCDKeypadRSTmod::LCDKeypadRSTmod() : LiquidCrystal(KEYPAD_LCD_PINS)
{
  keypadInit(KEYPAD_BACKLIGHT_PIN, KEYPAD_BUTTON_ANALOG_PIN);
}

LCDKeypadRSTmod::LCDKeypadRSTmod(uint8_t bl_pin, uint8_t button_apin) : LiquidCrystal(KEYPAD_LCD_PINS)
{
  keypadInit(bl_pin,button_apin);
}

void LCDKeypadRSTmod::keypadInit(uint8_t bl_pin, uint8_t button_apin)
{
  _bl_pin = bl_pin;
...
void LCDKeypadRSTmod::backlight()
{
...
void LCDKeypadRSTmod::noBacklight()
{
  // always try to keep the pin low
  digitalWrite(_bl_pin, LOW);
...
uint8_t LCDKeypadRSTmod::button()
{
  ...
  // add the RST button as the range between none and select...
  if (adc_key_in < KEYPAD_TRESHOLD_RST)  return KEYPAD_RST;
  return KEYPAD_NONE;  // when all others fail, return this...
}

uint8_t LCDKeypadRSTmod::buttonBlocking(uint16_t block_delay, uint16_t repeat_delay)
{
